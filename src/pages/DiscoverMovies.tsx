...
import { getDiscoverMovies } from '../services';
...
const DiscoverMovies: React.FC = () => {
  ...
  const fetchDiscoverMovies = useCallback(async (page: number) => {
    try {
      const { results, total_results } = await getDiscoverMovies({
        page,
      });
  ...
  }, [fetchDiscoverMovies, isApiCalled]);
  ...
  useEffect(() => {
    if (isLoadingMore) {
      fetchDiscoverMovies(page).then(() => setIsLoadingMore(false));
    }
  }, [isLoadingMore, fetchDiscoverMovies, page]);
